...
GRI2c::GRI2c(GRGpio& _sck,GRGpio& _sda)
	:m_iic_sck(_sck),m_iic_sda(_sda)
{
	m_sda_port=m_iic_sda.port();
	m_sda_pin=m_iic_sda.pin();
}
...
void GRI2c::sda_out()
{
	...
	if(p<8)
	{
		m_sda_port->CRL&=~(0xF	<<	(p<<2));
		m_sda_port->CRL|=	3	<<	(p<<2);
	}
	else
	{
		p-=8;
		m_sda_port->CRH&=~(0xF	<<	(p<<2));
		m_sda_port->CRH|=	3	<<	(p<<2);
	}
}
...
void GRI2c::sda_in()
{
	...
	{
		m_sda_port->CRL&=~(0xF	<<	(p<<2));
		m_sda_port->CRL|=	8	<<	(p<<2);
	}
	else
	{
		p-=8;
		m_sda_port->CRH&=~(0xF	<<	(p<<2));
		m_sda_port->CRH|=	8	<<	(p<<2);
	}
}
...
void GRI2c::iic_start()
{
	...
	GRCore::SleepUs(GRMacro::IIC_US);
 	m_iic_sda=0;//START:when CLK is high,DATA change form high to low 
	GRCore::SleepUs(GRMacro::IIC_US);
	m_iic_sck=0;//钳住I2C总线，准备发送或接收数据 
}	  
...
void GRI2c::iic_stop()
{
	sda_out();     //sda线输出
	m_iic_sck=1;
	m_iic_sda=0;//STOP:when CLK is high DATA change form low to high
 	GRCore::SleepUs(GRMacro::IIC_US);
	m_iic_sda=1;//发送I2C总线结束信号
	GRCore::SleepUs(GRMacro::IIC_US);		
	//iic_sck=1; 
}
//等待应答信号到来
...
u8 GRI2c::iic_wait_ack()
{
	u8 ucErrTime=0;
	sda_in();      //SDA设置为输入  
	m_iic_sck=1;GRCore::SleepUs(GRMacro::IIC_US); 
	m_iic_sck=1;GRCore::SleepUs(GRMacro::IIC_US);
	while(m_iic_sda)
	{
		++ucErrTime;
...
{                        
	u8 t;   
	sda_out(); 	    
	m_iic_sck=0;//拉低时钟开始数据传输
	for(t=0;t<8;t++)
	{              
			m_iic_sda=(txd&0x80)>>7;
			txd<<=1; 	  
